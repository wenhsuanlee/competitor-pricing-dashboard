...
```jsonc
{
  ...
  "categories": {
    // 每個品類帶 spec_fields:決定細項欄位的「顯示順序與欄名」,滑鼠/鍵盤各自不同。
    // 前端對照表依此動態長出欄位;scrape.py 依品類 SPEC_SCHEMA 產生。
    "mouse": { "label": "辦公滑鼠", "spec_fields": ["连接方式","分辨率DPI","传感器","手感","滚轮","颜色"], "items": [ /* ... */ ] },
    "kb":    { "label": "辦公鍵盤", "spec_fields": ["连接方式","按键数","按键类型","按键行程","按键寿命","供电续航"], "items": [ /* ... */ ] }
  }
}
```
每個 item:
```jsonc
{
  ...
  "weight": "141 g",
  "specs_detail": {           // 細項規格;鍵=該品類 spec_fields 的欄名,值為文字(沒有填 "—")
    "连接方式": "蓝牙 + Logi Bolt 接收器双模",
    "分辨率DPI": "200-8000 DPI"
    /* …其餘欄位依品類 spec_fields */
  },
